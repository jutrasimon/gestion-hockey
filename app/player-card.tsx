...
export function PlayerCard({p,selected,onSelect,onOpen,handle,ghost=false}:{p:Player;selected?:boolean;onSelect?:()=>void;onOpen?:()=>void;handle?:ReactNode;ghost?:boolean}){
 ...
 <motion.div className={`roster-content ${flipped?'show-back':''}`} key={`${p.id}-${flipped}`} initial={{opacity:.4,rotateY:-12}} animate={{opacity:1,rotateY:0}} transition={{duration:.22}}>
 ...
 <div className="dossier-health"><h4>{health.label}</h4><p>{health.history}</p></div>
 </div>:<>
