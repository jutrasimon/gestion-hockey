...
export function PlayerCard({p,selected,onSelect,onOpen,handle,ghost=false,initialFlipped=false,onFlip,previewRecord,previewHealth,template=false}:{p:Player;template?:boolean;previewRecord?:{gp:number;goals:number;assists:number};previewHealth?:ReturnType<typeof healthOf>;selected?:boolean;onSelect?:()=>void;onOpen?:()=>void;handle?:ReactNode;ghost?:boolean;initialFlipped?:boolean;onFlip?:(value:boolean)=>void}){
 ...
 const club=teams.find(t=>t.id===clubOf(p.id))!;const health=previewHealth??healthOf(p);const production=previewRecord??recordFor(p,initialScope.season,initialScope.phase)!;
 ...
 {handle}<div className="roster-toolbar"><span>{flipped?'PROFIL':`#${p.num}`} {!flipped&&<span>· 2028–29</span>}</span>{!ghost&&!template&&<FavoriteButton id={p.id} name={p.name}/>}</div>
 ...
 <div className="roster-club"><span className="club-monogram" aria-hidden="true">{template?'T':club.name.slice(0,1)}</span><span>{template?'Template':club.name}<small>{p.type}</small><small title={playStyleMeaning}>Style : {p.playStyle}</small></span></div>
 ...
 <div className="roster-attributes">{p.stats.map((v,i)=><div key={labels[i]} title={labels[i]}><span>{['MAN','TIR','PUI','PAT','IQ','DÉF','CŒ'][i]}</span><strong style={{color:statColor(v)}}>{v}</strong></div>)}</div>
