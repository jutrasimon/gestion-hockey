...
export function PlayerCard({p,selected,onSelect,handle,ghost=false}:{p:Player;selected?:boolean;onSelect?:()=>void;handle?:ReactNode;ghost?:boolean}){
 ...
 {!flipped?<><div className="roster-production" aria-label="Statistiques de saison régulière">{[['MJ',production.gp],['B',production.goals],['A',production.assists],['PTS',production.goals+production.assists]].map(([l,v])=><div key={l}><span>{l}</span><strong>{v}</strong></div>)}</div><div className="roster-attributes">{p.stats.map((v,i)=><div key={labels[i]} title={labels[i]}><span>{['MAN','TIR','PUI','PAT','IQ','CRÉ','CŒ'][i]}</span><strong>{v}</strong></div>)}</div></>:<div className="roster-back"><h3>Développement</h3><dl><div><dt>Potentiel</dt><dd>{p.potential}</dd></div><div><dt>Dans la ligue</dt><dd>{p.league} saisons</dd></div><div><dt>Au club</dt><dd>{p.team} saisons</dd></div><div><dt>Caractère</dt><dd>{p.trait}</dd></div></dl><h3>Santé</h3><p>{health.history}</p></div>}
 <div className="roster-contract"><strong>{money(p.salary)}</strong><span> / an · {p.years} ans</span></div><div className={`roster-health ${health.injured?'injured':''}`}>{health.injured?<ShieldAlert size={14}/>:<ShieldCheck size={14}/>} {health.label}</div>
