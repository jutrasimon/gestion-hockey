...
export function JuiceLab(){
 ...
 <div className="juice-stage"><div className="juice-stage-grid" aria-hidden="true"/>
 <motion.div className="juice-emblem" key={`emblem-${burst}`} initial={false} animate={reduced?{}:mode==='impact'&&burst?{x:[0,-9,9,-5,5,0],scale:[1,.91,1.08,1]}:burst?{scale:[1,.9,1.12,1],rotate:[0,-5,5,0]}:{}} transition={{duration:.48}}><span className="juice-emblem-number">H.</span><span>CLUB LAB</span></motion.div>
 {burst>0&&!reduced&&<motion.div key={burst} className="juice-shockwave" initial={{scale:.3,opacity:.8}} animate={{scale:2.8,opacity:0}} transition={{duration:.75}}/>}
 <ParticleBurst trigger={burst} power={power} reduced={reduced}/>
