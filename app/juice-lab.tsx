...
function ParticleBurst({trigger,power,reduced}:{trigger:number;power:number;reduced:boolean}){
 ...
 useEffect(()=>{
  ...
  // Canvas cannot resolve CSS var() strings. Resolve through a DOM color property
  // so derived relative colors and live palette edits work at every burst.
  const probe=document.createElement('span');el.parentElement!.appendChild(probe);
  const colors=['action-soft','vfx-blue','vfx-pink','vfx-gold','vfx-flash'].map(key=>{probe.style.color=`var(--palette-${key})`;return getComputedStyle(probe).color});
  probe.remove();
