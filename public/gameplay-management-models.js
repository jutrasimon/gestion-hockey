(function(root){
  ...
  const scoutPlayers={prospect:{name:'Mathis Tremblay',visibility:40,stats:[7,9,6,8,5,6],traits:['Ambitieux','Réceptif aux conseils'],health:'Ancienne entorse · surveillance conseillée'},star:{name:'Viktor Sokolov',visibility:88,stats:[10,12,8,7,10,6],traits:['Cupide','Compétitif'],health:'Épaule sensible · contrôle conseillé'}};
  ...
  function scout(state,playerId,action,offerId){
    ...
    const indexes=action==='shot'?[1,0]:action==='defense'?[0,4]:[3,2],radius=reliability>=85?1:reliability>=65?2:3;
    let findings;
