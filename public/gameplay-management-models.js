(function(root){
  ...
  const scoutPlayers={prospect:{name:'Mathis Tremblay',visibility:40,stats:[7,9,6,8,5,7,6],traits:['Ambitieux','Réceptif aux conseils'],health:'Ancienne entorse · surveillance conseillée'},star:{name:'Viktor Sokolov',visibility:88,stats:[10,12,8,7,10,5,6],traits:['Cupide','Compétitif'],health:'Épaule sensible · contrôle conseillé'}};
  ...
  function scout(state,playerId,action,offerId){
    ...
    if(!player||!offer||!['shot','skating','defense','mind','health'].includes(action))throw Error('Rapport inconnu');
    ...
    const indexes=action==='shot'?[1,0]:action==='defense'?[5,4]:[3,2],radius=reliability>=85?1:reliability>=65?2:3;
    let findings;
    if(action==='shot'||action==='skating'||action==='defense')findings=indexes.map(i=>({attribute:i,min:Math.max(1,player.stats[i]-radius),max:Math.min(15,player.stats[i]+radius)}));
