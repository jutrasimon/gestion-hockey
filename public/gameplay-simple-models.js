...
function trio(id){if(!['star','worker','rookie'].includes(id))throw Error('Ailier inconnu');const team=[M.players.gagnon,M.players.fortin,M.players[id]];const creation=team.reduce((s,p)=>s+p.stats[4],0)/3,finishing=Math.max(...team.map(p=>p.stats[1])),defense=team.reduce((s,p)=>s+p.stats[5],0)/3;return {team,creation,finishing,defense,strength:(creation+finishing+defense)/3};}
